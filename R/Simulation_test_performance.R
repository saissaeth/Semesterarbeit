...
# set.seed(8008)
#
# dgp1 <- dgp(p = pF_eta_x1_x2 , m = mF_x1, t = 0, ol = 0, model = "normal",
#             xmodel = "unif")
#
# sim1 <- simulate.dgp(object = dgp1, rho = 0, nsim = 1600L, d = 20L, nsimtest = 1000L)
# head(sim1)
# nrow(sim1)
#
#
...
selected_data <- dandl %>%
  ...
  distinct() %>%
  ungroup() %>%
  group_by(setup, repl, p, m, t, sd, ol, nsim, nsimtest, seed) %>%
...
apply_dgp_rho <- function(dim,seed,p, m, t, sd, ol) {
  ...
  names(pmd)[length(names(pmd))][1] <- "tau"
